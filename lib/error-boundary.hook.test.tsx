...
test("handleError forwards along async errors", async () => {
  function AsyncBomb() {
    ...
    React.useEffect(() => {
      if (explode) {
        setTimeout(() => {
          handleError(new Error("❌ Error.ErrorHandler:: Timeout "))
        })
      }
    })
  ...
  expect(firstLineOfError).toMatchInlineSnapshot(`"Error: Uncaught [Error: ❌ AsyncBomb  ]"`)
...
test("can pass an error to useErrorHandler", async () => {
  function AsyncBomb() {
    ...
    React.useEffect(() => {
      if (explode) {
        setTimeout(() => {
          setError(new Error("❌ Error.AsyncBomb "))
        })
      }
    })
    return <button onClick={() => setExplode(true)}>Async Bomb Test</button>
  }
  render(
    <ErrorBoundary FallbackComponent={ErrorFallback}>
      <AsyncBomb />
